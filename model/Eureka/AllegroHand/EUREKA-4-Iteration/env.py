...
class AllegroHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_rot, self.object_pos, self.goal_rot, self.goal_pos, self.object_angvel)
...
@torch.jit.script
def compute_reward(object_rot: torch.Tensor, object_pos: torch.Tensor, goal_rot: torch.Tensor, goal_pos: torch.Tensor, object_angvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    """
    Computes the reward for the task of making the hand spin the object to a target orientation.
    
    Args:
    - object_rot (torch.Tensor): The current rotation of the object as a quaternion.
    - object_pos (torch.Tensor): The current position of the object.
    - goal_rot (torch.Tensor): The target rotation of the object as a quaternion.
    - goal_pos (torch.Tensor): The target position of the object.
    - object_angvel (torch.Tensor): The current angular velocity of the object.
    
    Returns:
    - reward (torch.Tensor): The computed total reward.
    - reward_components (Dict[str, torch.Tensor]): A dictionary of individual reward components.
    """
    
    # Calculate the alignment between the object and the goal's rotation using quaternion dot product
    orientation_diff = torch.abs(torch.sum(object_rot * goal_rot, dim=-1))
    
    # Transform and normalize the orientation reward 
    orientation_temp = 25.0  # Adjusted Temperature parameter for orientation reward
    orientation_reward = torch.exp(orientation_temp * (orientation_diff - 1.0))
    
    # Introduce an angular velocity component to encourage faster orientation matching
    ang_vel_mag = torch.norm(object_angvel, p=2, dim=-1)
    angular_velocity_reward = ang_vel_mag / 15.0  # Scale down to an appropriate range
    
    # Introduce a distance component to encourage the object staying near the goal position
    distance_diff = torch.norm(object_pos - goal_pos, p=2, dim=-1)
    distance_reward = torch.exp(-distance_diff)  # Use negative exponential for smooth change with distance
    
    # Aggregate the total reward
    total_reward = orientation_reward + angular_velocity_reward + distance_reward
