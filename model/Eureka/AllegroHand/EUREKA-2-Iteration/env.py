...
def compute_reward(object_rot: torch.Tensor, object_pos: torch.Tensor, goal_rot: torch.Tensor, goal_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Re-adjusting temperature parameter to enhance learning gradient
    orientation_temp = 10.0  # Adjusted Temperature parameter for orientation reward
    orientation_reward = torch.exp(orientation_temp * (orientation_diff - 1.0))
    
    # Introduce a dynamic adjustment component to help the agent reach the goal orientation faster
    orientation_diff_penalty = torch.norm(object_rot - goal_rot, p=2, dim=-1)

    # Re-adjust the temperature parameter for the penalty component
    penalty_temp = 2.0
    orientation_penalty = torch.exp(-penalty_temp * orientation_diff_penalty)

    # Aggregate the total reward
    total_reward = orientation_reward + orientation_penalty
