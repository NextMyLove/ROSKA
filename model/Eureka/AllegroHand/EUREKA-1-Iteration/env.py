...
class AllegroHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_rot, self.object_pos, self.goal_rot, self.goal_pos)
...
@torch.jit.script
def compute_reward(object_rot: torch.Tensor, object_pos: torch.Tensor, goal_rot: torch.Tensor, goal_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    """
    Computes the reward for the task of making the hand spin the object to a target orientation with additional components.
    ...
    # Transform and normalize the orientation reward component
    orientation_temp = 5.0  # Adjusted Temperature parameter for orientation reward
    orientation_reward = torch.exp(orientation_temp * (orientation_diff - 1.0))
    
    # Position difference reward component (added to guide the agent towards the goal position)
    position_diff = torch.norm(object_pos - goal_pos, p=2, dim=-1)
    position_temp = 1.0  # Temperature parameter for position reward
    position_reward = torch.exp(position_temp * (-position_diff))
    
    # Penalize longer episode lengths to encourage faster task completion
    length_penalty_temp = 0.01  # Temperature parameter for length penalty
    length_penalty = torch.exp(-length_penalty_temp * position_diff)

    # Aggregate the total reward
    total_reward = orientation_reward + position_reward - length_penalty
