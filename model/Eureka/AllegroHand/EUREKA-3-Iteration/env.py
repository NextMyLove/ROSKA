...
def compute_reward(object_rot: torch.Tensor, object_pos: torch.Tensor, goal_rot: torch.Tensor, goal_pos: torch.Tensor, object_angvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    """
    Computes the reward for the task of making the hand spin the object to a target orientation with additional dynamic components.
    ...
    - goal_pose (torch.Tensor): The target position of the object.
    ...
    orientation_temp = 15.0  # Adjusted Temperature parameter for orientation reward
    ...
    angular_velocity_reward = ang_vel_mag / 10.0  # Scale down to an appropriate range
    
    # Removing the orientation penalty for simplicity
