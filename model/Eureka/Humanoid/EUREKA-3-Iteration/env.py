...
class HumanoidGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.targets, self.potentials, self.prev_potentials, self.up_vec, self.heading_vec)
...
@torch.jit.script
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, 
                   up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Constants for reward scaling and temperature
    speed_temp = 1.5
    ang_vel_penalty_weight = 0.2
    stability_weight = 1.0
    rotation_stability_weight = 2.0
    ...
    # Speed Reward
    forward_speed = velocity[:, 0]  # Assuming x-axis is the forward direction
    speed_reward = torch.exp(forward_speed / speed_temp)  # Apply exponential transformation with temperature

    # Penalize high angular velocities to encourage stable running
    angular_velocity_penalty = torch.norm(ang_velocity, p=2, dim=-1) * ang_vel_penalty_weight

    # Stability Reward: reward for keeping the torso upright
    torso_upright = up_vec[:, 2]  # Assuming z-axis represents upright direction
    stability_reward = (torso_upright.clamp(min=0.0) + 1.0) / 2.0 * stability_weight  # Normalize stability between 0.5 and 1

    # Rotation Stability Reward
    up_vec_normalized = up_vec / torch.norm(up_vec, p=2, dim=-1, keepdim=True)
    target_up_vec = torch.tensor([0.0, 0.0, 1.0], device=up_vec.device).expand_as(up_vec_normalized)
    rotation_diff = torch.norm(up_vec_normalized - target_up_vec, p=2, dim=-1)
    rotation_stability_reward = (1.0 - rotation_diff).clamp(min=0.0) * rotation_stability_weight

    # Total reward
    total_reward = speed_reward - angular_velocity_penalty + stability_reward + rotation_stability_reward

    return total_reward, {
        'speed_reward': speed_reward,
        'angular_velocity_penalty': angular_velocity_penalty,
        'stability_reward': stability_reward,
        'rotation_stability_reward': rotation_stability_reward,
    }
