...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, 
    ...
    angular_velocity_penalty = torch.norm(ang_velocity, p=2, dim=-1) * 0.05  # Reduced weight

    # Stability Reward: reward for keeping the torso upright
    torso_upright = up_vec[:, 2]  # Assuming z-axis represents upright direction
    stability_reward = torso_upright.clamp(min=0.0)  # Reward capped at a minimum of 0

    total_reward = speed_reward - angular_velocity_penalty + stability_reward * 0.5  # Balanced weight for stability

    return total_reward, {
        'speed_reward': speed_reward,
        'angular_velocity_penalty': angular_velocity_penalty,
        'stability_reward': stability_reward * 0.5
    }
