...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, 
                   prev_potentials: torch.Tensor, up_vec: torch.Tensor, heading_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Constants for reward scaling and temperature
    speed_temp = 2.0
    ang_vel_penalty_weight = 0.5
    stability_weight = 1.0
    rotation_stability_weight = 3.0
    rotation_temperature = 0.5
    ...
    # Rotation Stability reward using a temperature parameter to control the sharpness
    ...
    rotation_stability_reward = torch.exp((1.0 - rotation_diff) * rotation_temperature).clamp(max=1.0) * rotation_stability_weight
