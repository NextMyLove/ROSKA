...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, 
    ...
    # Reward component for forward velocity (running speed)
    forward_speed = velocity[:, 0]  # Assuming x-axis is the forward direction
    speed_reward = torch.exp(forward_speed / 2.0)  # Apply exponential transformation with temperature

    # Penalize high angular velocities to encourage stable running
    angular_velocity_penalty = torch.norm(ang_velocity, p=2, dim=-1) * 0.02  # Further reduced weight

    # Stability Reward: reward for keeping the torso upright and minimal rotation
    torso_upright = up_vec[:, 2]  # Assuming z-axis represents upright direction
    stability_reward = (torso_upright.clamp(min=0.0) + 1.0) / 2.0  # Normalize stability between 0.5 and 1

    # Stability Reward: Adding rotational stability
    ...
    rotation_stability_reward = (1.0 - rotation_diff).clamp(min=0.0)
