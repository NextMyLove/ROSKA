...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Calculate the difference between current rotation and goal rotation
    rotation_diff = torch.norm(object_rot - goal_rot, p=2, dim=-1)
    
    # Reward is negatively proportional to the rotational difference
    reward = -rotation_diff

    # Normalize the reward using an exponential transformation
    temp_rotation_diff = 1.0  # Temperature parameter for rotation difference
    norm_reward = torch.exp(reward / temp_rotation_diff)

    # Prepare the individual reward components dictionary
    reward_components = {
        'rotation_diff_reward': reward
    }
    
    return norm_reward, reward_components
