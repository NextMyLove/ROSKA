...
class AllegroHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_rot, self.goal_rot, self.object_pos, self.goal_pos, self.object_linvel)
...
@torch.jit.script
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, object_pos: torch.Tensor, goal_pos: torch.Tensor, object_linvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Scaled orientation distance component
    orientation_distance = torch.norm(object_rot - goal_rot, p=2, dim=-1)
    orientation_reward_weight = 5.0  # Increased weight for more significance
    orientation_reward = -orientation_distance * orientation_reward_weight
    
    # Transformed using an adjusted temperature 
    temperature_orientation = 1.5  # Adjusted temperature
    transformed_orientation_reward = torch.exp(orientation_reward / temperature_orientation)

    # Scaled positional reward to keep object close to target position
    position_distance = torch.norm(object_pos - goal_pos, p=2, dim=-1)
    position_reward_weight = 5.0  # Increased weight for more influence
    position_reward = -position_distance * position_reward_weight

    # Transformation for position reward
    temperature_position = 1.5  # Adjusted temperature
    transformed_position_reward = torch.exp(position_reward / temperature_position)

    # Penalty for high velocities to encourage stability
    velocity_penalty_weight = 1.0  # Increased weight
    velocity_penalty = -torch.norm(object_linvel, p=2, dim=-1) * velocity_penalty_weight

    # Additional reward for maintaining closeness to the goal rotation
    stable_rotation_proximity_weight = 2.0  # Increased weight
    stable_rotation_proximity_reward = torch.exp(-orientation_distance * stable_rotation_proximity_weight)

    # Calculate the total reward
    total_reward = (transformed_orientation_reward + transformed_position_reward + 
                    velocity_penalty + stable_rotation_proximity_reward)

    # Return the total reward and the dictionary of each reward component
    reward_components = {
        "orientation_reward": orientation_reward,
        "transformed_orientation_reward": transformed_orientation_reward,
        "position_reward": position_reward,
        "transformed_position_reward": transformed_position_reward,
        "velocity_penalty": velocity_penalty,
        "stable_rotation_proximity_reward": stable_rotation_proximity_reward
    }
