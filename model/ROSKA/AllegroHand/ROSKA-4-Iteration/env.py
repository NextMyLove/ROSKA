...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, object_pos: torch.Tensor, goal_pos: torch.Tensor, object_linvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Scaled orientation distance
    orientation_distance = torch.norm(object_rot - goal_rot, p=2, dim=-1)
    orientation_reward_weight = 10.0  # Increased weight for more significance
    orientation_reward = -orientation_distance * orientation_reward_weight

    # Transformed with adjusted temperature
    temperature_orientation = 2.0  # Adjusted temperature
    transformed_orientation_reward = torch.exp(orientation_reward / temperature_orientation)

    # Scaled positional reward
    position_distance = torch.norm(object_pos - goal_pos, p=2, dim=-1)
    position_reward_weight = 10.0  # Increased weight for more influence
    position_reward = -position_distance * position_reward_weight

    # Linear transformation for position reward (instead of exp)
    transformed_position_reward = torch.clamp(position_reward + 1, min=0.0)

    # Penalty for high velocities
    velocity_penalty_weight = 1.5  # Slightly increased weight
    velocity_penalty = -torch.norm(object_linvel, p=2, dim=-1) * velocity_penalty_weight

    # Stable rotation proximity reward
    stable_rotation_proximity_weight = 3.0  # Increased weight for better alignment payoff
