...
def compute_reward(root_states: torch.Tensor, dof_vel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Adjusted reward settings
    temp_velocity = 0.5  # Slightly decreased to reduce over-dominance
    penalty_ang_vel_factor = 0.3  # Increased penalty factor for stronger stability incentive
    upright_posture_penalty_factor = 6.0  # Increased to have more significant influence
    ...
    # Using quaternion for more comprehensive upright evaluation
    upright_diff = torch.abs(torso_rotation[:, 2])  # Penalizes deviation in z (up) direction
    upright_posture_penalty = -upright_posture_penalty_factor * upright_diff
