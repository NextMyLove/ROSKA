...
def compute_reward(root_states: torch.Tensor, dof_vel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    forward_velocity = velocity[:, 0]  # Assuming 'x' direction is forward

    # Reward settings - these are modified
    temp_velocity = 0.55  # Adjusted to increase impact
    temp_ang_vel_penalty = 0.2  # Increased to penalize more
    penalty_ang_vel_factor = 0.2  # Increased penalty factor
    upright_posture_penalty_factor = 4.0  # Increased to penalize more

    # Compute forward velocity reward
    forward_velocity_reward = torch.exp(temp_velocity * forward_velocity)

    # Penalty for high angular velocities (to encourage stability)
    ang_velocity_penalty = -penalty_ang_vel_factor * torch.norm(ang_velocity, p=2, dim=-1)

    # Penalty for deviation from upright posture (to encourage running upright)
    # Assuming z-axis is upward and torso_rotation is a quaternion
    upright_posture_penalty = -upright_posture_penalty_factor * torch.abs(torso_rotation[:, 2])  # Penalizes deviation in z (up) direction

    total_reward = forward_velocity_reward + ang_velocity_penalty + upright_posture_penalty

    reward_components = {
        "forward_velocity_reward": forward_velocity_reward,
        "ang_velocity_penalty": ang_velocity_penalty,
        "upright_posture_penalty": upright_posture_penalty
    }
