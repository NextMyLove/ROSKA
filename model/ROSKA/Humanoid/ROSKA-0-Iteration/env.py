...
def compute_reward(root_states: torch.Tensor, dof_vel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Reward settings - these can be tuned
    temperature_velocity = 0.1
    temperature_ang_vel = 0.1
    penalty_ang_vel = 0.1

    # Compute forward velocity reward
    forward_velocity_reward = torch.exp(temperature_velocity * forward_velocity)

    # Penalty for high angular velocities (to encourage stability)
    ang_velocity_penalty = -penalty_ang_vel * torch.sum(ang_velocity ** 2, dim=-1)

    total_reward = forward_velocity_reward + ang_velocity_penalty
